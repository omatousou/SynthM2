...
- **`__init__(fs=44100)`**
  ...
  - Paramètre `fs` : fréquence d'échantillonnage en Hz
...
- **`get_block(freqs, phases, duration, wave_type)`**
  ...
  - Retourne : tuple (t, audio_data)
    - `t` : vecteur temps
    - `audio_data` : tableau int16 contenant les échantillons audio
...
freqs = [440, 880]  # La et La octave supérieure
phases = {440: 0, 880: 0}
t, data = gen.get_block(freqs, phases, duration=0.05, wave_type="Sinus")
```
...
### Description
Interface graphique PyQt5 du synthétiseur. Permet à l'utilisateur de contrôler le synthétiseur via le clavier AZERTY et de visualiser l'onde en temps réel.
